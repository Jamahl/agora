...
7. **Synthesis** (async, ~10–20 s):
   ...
   4. **OKR-tag** — cosine similarity shortlists active OKRs/KRs, then a strict structured LLM classifier decides material business relevance. Each insight gets at most one KR: the highest-similarity judge-approved KR, plus its parent `insight_okr_tag`. If no KR is approved, the best approved objective can still create an objective-only tag. If the classifier fails, synthesis falls back to the strongest threshold-qualified semantic match (`okr_tag_threshold`, default 0.55; KR threshold maxes to at least 0.65).
...
- **OKR scope + KR signal** — `okr.scope_type/scope_id` support company vs department OKRs. `insight_key_result_tag` stores one high-confidence KR-level link per insight, created during synthesis. Matching now uses embeddings for candidate recall and a strict structured LLM pass for final relevance, then keeps the highest-similarity approved KR so terse KRs like "10 acquisitions" can catch concrete operational signals like slow deal review without spraying the same feedback across multiple KRs.
...
- **OKR tagging now uses embedding recall + LLM judgment, but still depends on OKR clarity.** Synthesis embeds every extracted insight, shortlists active objective/KR candidates, and asks a strict structured LLM classifier whether the insight materially affects each candidate. It keeps only the highest-similarity approved KR per insight, avoiding duplicate attribution while still catching human-obvious links that embeddings miss, such as deal-review delays affecting an acquisitions KR. Very vague OKRs still reduce explainability; prefer concrete KR descriptions and keep a visible "unlinked signal" explanation in the OKR UI.
